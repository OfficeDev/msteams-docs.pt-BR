...
Quando um bot do Teams recebe uma atividade, ela é roteado para os manipuladores de atividades. Todas as atividades são roteadas por meio de um manipulador base chamado manipulador de turnos. O manipulador de turnos chama o manipulador de atividades necessário para gerenciar qualquer atividade recebida. O bot do Teams é derivado da classe `TeamsActivityHandler`, que é derivada da classe `ActivityHandler` do Bot Framework.
...
Para implementar sua lógica de manipuladores de atividades específicos do Teams, você deve substituir os métodos em seu bot, conforme mostrado na seção [lógica do bot](#bot-logic). Não há nenhuma implementação base para esses manipuladores, portanto, você deve adicionar a lógica desejada durante a substituição.
...
A lógica do bot processa atividades de entrada de um ou mais de seus canais de bot e, em resposta, gera atividades de saída. Isso ainda se aplica aos bots derivados da classe do manipulador de atividades do Teams, que primeiro verifica as atividades do Teams. Depois de verificar as atividades do Teams, ele passa todas as outras atividades para o manipulador de atividades do Bot Framework.
...
A lista de manipuladores definidos na `ActivityHandler` inclui o seguinte:
...
A lista de manipuladores de atividades do Teams chamada do manipulador de atividades `OnInvokeActivityAsync` do Teams inclui o seguinte:
...
| actionableMessage/executeAction | `OnTeamsO365ConnectorCardActionAsync` | Esse método é invocado quando uma atividade de ação de cartão do conector do Office 365 é recebida do conector. |
...
A lista de manipuladores de atividades do Teams chamada do manipulador de atividades `onInvokeActivity` do Teams inclui o seguinte:
...
| actionableMessage/executeAction | `handleTeamsO365ConnectorCardAction` | Esse método é invocado quando uma atividade de ação de cartão do conector do Office 365 é recebida do conector. |
...
| Outros tipos de atividade recebidas | `on_unrecognized_activity_type` | Esse método pode ser substituído para manipular qualquer tipo de atividade que não seja manipulada. |
...
A lista de manipuladores de atividades do Teams chamada do manipulador de atividades `on_invoke_activity` do Teams inclui o seguinte:
...
| actionableMessage/executeAction | `on_teams_o365_connector_card_action` | Esse método é invocado quando uma atividade de ação de cartão do conector do Office 365 é recebida do conector. |
...
Agora que você se familiarizou com os manipuladores de atividades de bot, vamos ver como os bots se comportam de forma diferente dependendo da conversa e das mensagens que ele recebe ou envia.
