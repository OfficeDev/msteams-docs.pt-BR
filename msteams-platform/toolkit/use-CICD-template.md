...
Além disso, você precisa criar pipelines CI ou CD no Jenkins que apontem para o específico `Jenkinsfile` correspondentemente.
...
1. Renomeie o arquivo de modelo para `Jenkinsfile`e coloque-o no branch de destino, por exemplo, o `main` branch.
1. Altere o fluxo de CD. Padrão é usar os gatilhos de quando `pollSCM` uma nova alteração é empurrada para o `main` branch.
...
Se houver alterações relacionadas Teams arquivo de manifesto do aplicativo, talvez você queira publicar o aplicativo Teams novamente para atualizar o manifesto.
