...
| Ignorar a adição de usuário durante o provisionamento SQL | <ul> <li>`skipAddingSqlUser` property in`.fx/config.<envName>.json`.</li> </ul> | [Ignore a adição de usuário para SQL banco de dados](provision.md#skip-adding-user-for-sql-database). |
| Personalizar manifesto do aplicativo | <ul> <li>`templates/manifest.remote.template.json`.</li> <li>`manifest` section in`.fx/config.<envName>.json`.</li>  </ul> | [Personalize Teams Manifesto do Aplicativo no Teams Toolkit](TeamsFx-manifest-customization.md). |
...
Neste cenário, você aprenderá a definir diferentes Teams de aplicativo para diferentes ambientes:
