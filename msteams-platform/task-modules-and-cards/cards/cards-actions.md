...
>* Se você estiver usando uma ação de cartão como parte de uma extensão de mensagens, as ações não funcionarão até que o cartão seja enviado ao canal. Eles não funcionam enquanto o cartão está na caixa de mensagem de composição.
...
| `value` | De acordo com a URL para a que você deseja redirecionar.  |
