...
|`successCallback`| A função é chamada se a autenticação for bem-sucedida, com o resultado retornado do pop-up de autenticação. O Authcode é o resultado.|
|`url`  <br>|A URL do servidor de aplicativos 3P para o pop-up de autenticação, com os dois espaços reservados de parâmetro a seguir:</br> <br> - `oauthRedirectMethod`: Espaço reservado de aprovação em `{}`. Esse espaço reservado é substituído por um deeplink ou uma página da Web pela plataforma Teams, que informa ao servidor de aplicativos se a chamada está vindo da plataforma móvel.</br> <br> - `authId`: Esse espaço reservado é substituído por UUID. O servidor de aplicativos o usa para manter a sessão.| 
...
### <a name="steps-to-perform-authentication-to-external-browsers"></a>Etapas para realizar a autenticação em navegadores externos

<!-- #### 1. Pass `isExternal` and placeholders in `url` -->
**1. Inicie o processo de logon de autenticação externo**
...
**2. Link do Teams em um navegador**
...
**3. A resposta do servidor de aplicativos 3P**
...
**4. O servidor de aplicativo 3P redireciona para o especificado`url`**
...
**5. Entrar no navegador externo**

O usuário se conecta ao navegador externo. Os provedores OAuth redirecionam de volta para a `redirect_uri` com o código de autenticação e o objeto de estado.

**6. A resposta do servidor de aplicativos 3P ao Teams** 
 ...
 **8. Resultados de retorno de chamada bem-sucedidos do Teams**
