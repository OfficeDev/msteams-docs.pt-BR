...
3. O Teams solicita o token de acesso da guia do ponto de extremidade do Microsoft Azure Active Directory (Microsoft Azure Active Directory (Azure AD)) para o usuário atual.
4. O Microsoft Azure Active Directory (Microsoft Azure Active Directory (Azure AD)) envia o token de acesso da guia ao aplicativo Teams.
...
### <a name="1-create-your-microsoft-azure-active-directory-azure-ad-application"></a>1. Crie o seu aplicativo do Microsoft Azure Active Directory (Azure AD)
...
> * É importante que o nome de domínio do seu aplicativo seja igual ao nome de domínio que você registrou para o seu aplicativo do Microsoft Azure Active Directory (Azure AD).
...
**Para registrar o seu aplicativo por meio do portal do Microsoft Azure Active Directory (Azure AD)**

1. Registre um novo aplicativo no portal de [Registros de Aplicativos do Microsoft Azure Active Directory do Microsoft Azure Active Directory (Azure AD](https://go.microsoft.com/fwlink/?linkid=2083908)).
...
> [!NOTE]
>
> * ¹ Se o aplicativo do Microsoft Azure Active Directory (Azure AD) estiver registrado no mesmo locatário em que você está fazendo uma solicitação de autenticação no Teams, o usuário não pode ser solicitado a consentir e recebe um token de acesso imediatamente. Os usuários só consentem com essas permissões se o aplicativo do Microsoft Azure Active Directory (Azure AD) estiver registrado em um locatário diferente.
> * ² Se o domínio personalizado não for adicionado ao Microsoft Azure Active Directory (Azure AD), você receberá um erro informando que o nome do host não deve ser baseado em um domínio já pertencente. Para adicionar um domínio personalizado ao Microsoft Azure Active Directory (Azure AD) e registrá-lo, siga o procedimento [adicionar um nome de domínio personalizado ao Microsoft Azure Active Directory (Azure AD)](/azure/active-directory/fundamentals/add-custom-domain) e repita a etapa 5. Você também verá esse erro se você não estiver conectado com as credenciais do administrador no locatário do Office 365.
> * Se você não estiver recebendo o nome UPN no token de acesso retornado, poderá adicioná-lo como uma [declaração opcional](/azure/active-directory/develop/active-directory-optional-claims) no Microsoft Azure Active Directory (Azure AD).
...
> [!div class="checklist"]
> * **id** - A ID do cliente do aplicativo. Esta é a ID do aplicativo que você obteve como parte do registro do aplicativo no Microsoft Azure Active Directory (Azure AD).
...
> [!NOTE]
>
>* O recurso para um aplicativo do Microsoft Azure Active Directory (Azure AD) geralmente é a raiz da URL do site e o appID (por exemplo. `api://subdomain.example.com/00000000-0000-0000-0000-000000000000`). Esse valor também é usado para garantir que sua solicitação venha do mesmo domínio. Verifique se `contentURL` para sua guia usa os mesmos domínios que sua propriedade de recurso.
...
Outra abordagem para obter escopos de gráfico é apresentar uma caixa de diálogo de consentimento usando nossa [abordagem de autenticação do Microsoft Azure Active Directory (Azure AD) baseada na Web existente](~/tabs/how-to/authentication/auth-tab-aad.md#navigate-to-the-authorization-page-from-your-pop-up-page). Essa abordagem envolve a exibição de uma caixa de diálogo de consentimento do Microsoft Azure Active Directory (Azure AD).

**Para solicitar consentimento adicional usando a API Auth**

1. O token recuperado usando `getAuthToken()` deve ser trocado no lado do servidor usando o [fluxo em nome](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) do Microsoft Azure Active Directory (Azure AD) para obter acesso a essas outras APIs do Graph. Certifique-se de usar o ponto de extremidade v2 Graph para esta troca.
2. Se a troca falhar, o Microsoft Azure Active Directory (Azure AD) retornará uma exceção de concessão inválida. Geralmente, há uma das duas mensagens de erro, `invalid_grant` ou `interaction_required`.
3. Quando a troca falhar, você deve solicitar consentimento. Exiba uma interface do usuário (UI) solicitando que o usuário conceda outro consentimento. Essa interface do usuário deve incluir um botão que aciona uma caixa de diálogo de consentimento do Microsoft Azure Active Directory (Azure AD) usando nossa [API de autenticação do Microsoft Azure Active Directory (Azure AD)](~/concepts/authentication/auth-silent-aad.md).
4. Ao solicitar mais consentimento do Microsoft Azure Active Directory (Azure AD), você deve incluir `prompt=consent` em seu [parâmetro da cadeia de caracteres de consulta](~/tabs/how-to/authentication/auth-silent-aad.md#get-the-user-context) para o Microsoft Azure Active Directory (Azure AD), caso contrário, o Microsoft Azure Active Directory (Azure AD) não solicitará o outros escopos.
...
### <a name="non-microsoft-azure-active-directory-azure-ad-authentication"></a>Autenticação não baseada no Microsoft Azure Active Directory (Azure AD)

A solução de autenticação descrita acima só funciona para aplicativos e serviços que dão suporte ao Microsoft Azure Active Directory (Azure AD) como um provedor de identidade. Os aplicativos que desejam autenticar usando serviços não baseados no Microsoft Azure Active Directory (Azure AD) devem continuar usando o [fluxo de autenticação da Web](~/concepts/authentication.md) baseado em pop-up.

> [!NOTE]
> O SSO é compatível com aplicativos de propriedade do cliente nos locatários B2C do Microsoft Azure Active Directory (Azure AD).
