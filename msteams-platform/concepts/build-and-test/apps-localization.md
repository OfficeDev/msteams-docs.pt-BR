...
description: Descreve as considerações para localizar seu Microsoft Teams aplicativo.
ms.topic: conceptual
...
1. [Tratar envios de texto localizados de seus usuários](#handle-localized-text-submissions-from-your-users).

## <a name="localize-your-appsource-listing"></a>Localizar sua listagem do AppSource
...
Use o Microsoft Teams de aplicativo `v1.5` e posterior para localizar seu aplicativo. Você pode fazer isso definindo `$schema` o atributo em seu arquivo manifest.json `$schema` `https://developer.microsoft.com/json-schemas/teams/v1.5/MicrosoftTeams.schema.json` `manifestVersion` como ou superior e atualizando a propriedade para versão (`1.5`nesse caso).

Adicione a `localizationInfo` propriedade com o idioma padrão ao qual seu aplicativo dá suporte. O idioma padrão será usado como o idioma de fallback final se as configurações do cliente do usuário não corresponderem a nenhum dos seus idiomas adicionais.
...
Você pode fornecer arquivos .json adicionais com traduções de todas as cadeias de caracteres voltadas para o usuário em seu manifesto. Esses arquivos devem aderir ao esquema [JSON](../../resources/schema/localization-schema.md) do arquivo de localização e devem ser `localizationInfo` adicionados à propriedade do manifesto. Cada arquivo se correlaciona a uma marca de idioma, que o Teams cliente usa para selecionar as cadeias de caracteres apropriadas. A marca de idioma assume a forma, `<language>-<region>` mas você pode omitir `<region>` a parte para direcionar todas as regiões que dão suporte ao idioma desejado.

O cliente Teams aplica as cadeias de caracteres na seguinte ordem: cadeias de caracteres de idioma padrão > - cadeias de caracteres somente de idioma do usuário -> idioma do usuário + cadeias de caracteres de região do usuário.

Por exemplo, você fornece um idioma padrão de 'fr' (francês, todas as regiões) e arquivos de idioma adicionais para 'en' (inglês, todas as regiões) e 'en-gb' (inglês, Grã-Bretanha), o idioma do usuário é definido como 'en-gb'. As seguintes alterações ocorrem com base na seleção de idioma:
...
Portanto, você deve fornecer traduções somente de idioma de nível superior em seu manifesto. Por exemplo, em `en` vez de `en-us`. Você deve fornecer substituições no nível da região somente para as poucas cadeias de caracteres que precisam delas.

### <a name="example-manifestjson-change"></a>Exemplo de alteração de manifest.json

A `manifest.json` alteração é mostrada no exemplo a seguir:
...
### <a name="example-localization-json-file"></a>Exemplo de arquivo .json de localização
 ...
 A `localization.json` alteração é mostrada no exemplo a seguir:
...
|-------------|-------------|------|------|
| Localização do aplicativo | Microsoft Teams localização do aplicativo usando bot e guia. | [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/app-localization/csharp) |[Exibir](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/app-localization/nodejs) |
