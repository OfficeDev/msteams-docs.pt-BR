...
Este documento orienta você sobre como integrar recursos de mídia. Essa integração combina os recursos nativos do dispositivo, como a **câmera** e o **microfone** com a plataforma do Teams.  

Você pode usar o [SDK](/javascript/api/overview/msteams-client?view=msteams-client-js-latest&preserve-view=true)do cliente JavaScript do Microsoft Teams, que fornece as ferramentas necessárias para que seu aplicativo acesse as permissões de dispositivo [de um usuário.](native-device-permissions.md) Use **APIs** de recursos de mídia adequadas para  integrar  os recursos nativos do dispositivo, como a câmera e o microfone com a plataforma do Teams no aplicativo móvel do Microsoft Teams, e crie uma experiência mais rica. 

## <a name="advantage-of-integrating-media-capabilities"></a>Vantagem da integração de recursos de mídia

A principal vantagem da integração de recursos de dispositivo em seus aplicativos do Teams é que ele aproveita os controles nativos do Teams para fornecer uma experiência rica e imersiva aos usuários.
...
Para uma integração eficaz, você deve ter um bom entendimento de trechos de código para chamar as [respectivas](#code-snippets) APIs, que permitem que você use recursos de mídia nativa.
...
Atualize seu aplicativo [ teamsmanifest.jsno](../../resources/schema/manifest-schema.md#devicepermissions) arquivo adicionando a propriedade e `devicePermissions` especificando `media` . Ele permite que seu aplicativo peça permissões de requisito dos  usuários antes que eles comecem a usar a câmera para capturar  a imagem, abra a galeria para selecionar uma imagem para enviar como um anexo ou use o microfone para gravar a conversa.
...
> [!NOTE]
> O **prompt solicitação de permissões** é exibido automaticamente quando uma API relevante do Teams é iniciada. Para obter mais informações, consulte [Solicitar permissões de dispositivo.](native-device-permissions.md)

## <a name="media-capability-apis"></a>APIs de recursos de mídia
...
* Use o **controle nativo da** câmera para permitir que os usuários **capturem e anexem imagens** em qualquer lugar.
* Use o **suporte nativo à galeria** para permitir que os usuários **selecionem imagens de dispositivo** como anexos.
* Use o controle **do visualizador de imagem nativo** para visualizar várias **imagens** ao mesmo tempo.
* Suporte **a transferência de imagem grande** (de 1 MB a 50 MB) por meio da ponte do SDK.
* Suporte **a recursos avançados de imagem,** permitindo que os usuários visualizem e editem imagens:
  * Digitalizar documentos, quadro de visita e cartões de visita por meio da câmera.
  
> [!IMPORTANT]
>*   As APIs , e podem ser invocadas de várias superfícies do Teams, como módulos de `selectMedia` `getMedia` `viewImages` tarefas, guias e aplicativos pessoais. Para obter mais detalhes, confira [Pontos de entrada para aplicativos do Teams.](../extensibility-points.md)
...
**Experiência do aplicativo Web para a API selectMedia para funcionalidade de imagem** 
 ![ experiência de câmera e imagem do dispositivo no Teams](../../assets/images/tabs/image-capability.png)
...
Você deve garantir o tratamento adequado desses erros em seu aplicativo do Teams. A tabela a seguir lista os códigos de erro e as condições sob as quais os erros são gerados: 
...
| **6000** |INSUFFICIENT_RESOURCES | Não foi possível concluir a operação devido a recursos insuficientes.|
|**7000** | THROTTLE | A plataforma acelerou a solicitação, pois a API era invocada com frequência.|
|  **8000** | USER_ABORT |O usuário anula a operação.|
...
**Chamada `getMedia` API** para recuperar mídia grande em partes:
