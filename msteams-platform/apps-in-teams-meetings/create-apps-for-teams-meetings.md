...
A `GetParticipant` API permite que um bot busque informações do participante por meio da ID da reunião e da ID do participante. A API inclui parâmetros de consulta, exemplos e códigos de resposta. A API tem suporte em reuniões agendadas ou recorrentes privadas e reuniões agendadas ou recorrentes no canal. 
...
O corpo da resposta JSON para a `GetParticipant` API é:
...
A `Meeting Details` API permite que seu aplicativo receba metadados de reunião estáticos. Os metadados fornece pontos de dados que não mudam dinamicamente.
A API está disponível por meio dos Serviços bot.

#### <a name="prerequisite"></a>Pré-requisito

Para usar a `Meeting Details` API, você deve obter permissões RSC. Use o exemplo a seguir para configurar a propriedade do manifesto do `webApplicationInfo` aplicativo:
...
A `Meeting Details` API inclui o seguinte parâmetro de consulta:
...
A `Meeting Details` API inclui os seguintes exemplos:
...
O corpo da resposta JSON para `Meeting Details` a API é o seguinte:
...
A hora real de início e término de uma reunião é diferente da hora de início e término agendada. A `Meeting Details` API fornece a hora de início e término agendada. O evento fornece a hora real de início e término.
